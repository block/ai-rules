...
impl CommandGeneratorTrait for CursorCommandGenerator {
    fn generate_commands(&self, current_dir: &Path) -> HashMap<PathBuf, String> {
        ...
        let commands_dir = current_dir.join(CURSOR_COMMANDS_DIR).join(GENERATED_COMMANDS_SUBDIR);
    ...
    fn clean_commands(&self, current_dir: &Path) -> Result<()> {
        let commands_subdir = current_dir.join(CURSOR_COMMANDS_DIR).join(GENERATED_COMMANDS_SUBDIR);
    ...
    fn command_gitignore_patterns(&self) -> Vec<String> {
        vec![format!("{}/{}/", CURSOR_COMMANDS_DIR, GENERATED_COMMANDS_SUBDIR)]
    }
}
...
mod tests {
    ...
    fn test_generate_commands_strips_frontmatter() {
        ...
        let output_path = temp_dir.path().join(CURSOR_COMMANDS_DIR).join("ai-rules").join("test.md");
